...
const LoginDoc = () => {
    ...
    const handleLogin = async () => {
        ...
        // try {
        //     setLoading(true)
        //     const data = await AuthAPI.postPhoneVerifyOtp({
        //         "phone_no":Number(email),
        //         "otp": Number(otp),
        //     })
        //     console.log(data);
    ...
    const handleSendOTP = async() => {
        ...
        try {
            setLoading(true)
            const data = await AuthAPI.postPhone({
                "phone_no": email,
            })
            ...
            <div className='w-1/2 container mx-auto flex flex-col items-center mt-[200px]'>
                ...
                <div className='flex flex-col gap-[10px] card w-[80%]md:w-[40%] shadow-[0px_0px_8px_rgba(0,0,0,0.1)] bg-[#ffffff] py-[30px] px-[20px] rounded-[8px]  mx-auto'>
                    ...
                    <div className='flex flex-col gap-[10px] w-[100%] mt-[30px] mx-auto'>
                        ...
                        < Input label="Password" name="otp" type="password" placeholder="Enter your password" className=" rounded-[12px]" value={otp} handleChange={(e: any) => setOtp(e.target.value)} />
                                <button className="bg-[#7F32AC] text-[#fff] text-[14px] px-[45px] mx-auto font-semibold rounded-[12px] py-[8px] mt-[15px]" onClick={handleLogin}>
                                Sign In 
                                </button>
                    ...
                    {/* <p className='text-[#000] text-center text-[#000]/[0.7] mt-[15px] font-medium'>Want to Sign In with UID? <span className='text-right text-[#7F32AC] hover:underline cursor-pointer' onClick={() => {
                        navigate(LOGIN)
                    }}>Click here</span>.</p> */}
                </div>
